...
int MainWindow::OnCreate(LPCREATESTRUCT /*createStruct*/)
{
    ...
    m_renderer = std::make_unique<ImageRenderer>(m_hWnd, m_d2dFactory.get());
...
void MainWindow::DisplayImage(const std::wstring& displayName, LoadedImage image)
{
    ...
    m_renderer->ClearTiles();  // recreated from m_cpuImage on next render
...
{
    if (m_renderer && size.cx > 0 && size.cy > 0)
    {
        const HRESULT hr = m_renderer->Resize(size.cx, size.cy);
        if (ImageRenderer::IsDeviceLost(hr))
        {
            m_renderer->DiscardDevice();
        }
        else
        {
            LOG_IF_FAILED(hr);
...
{
    if (!m_renderer || FAILED(m_renderer->EnsureDevice()))
    {
        return;
    }

    if (!m_renderer->HasTiles() && m_cpuImage)
    {
        const HRESULT hr = m_renderer->UploadImage(m_cpuImage);
        if (FAILED(hr))
        {
    ...
    const ViewLayout* layoutPtr = nullptr;
    ViewLayout layout;
    if (m_renderer->HasTiles())
    {
    ...
    ImageRenderer::Overlay overlay;
    overlay.dpiScale = DpiScale();
    if (m_state == ViewState::Loading || m_state == ViewState::Error)
    {
        overlay.statusText = m_statusText.c_str();
        overlay.statusLength = static_cast<UINT32>(m_statusText.size());
        overlay.textFormat = m_textFormat.get();
    }
    if (layoutPtr && InSelectionMode() && m_selection.HasRect())
    {
        overlay.showSelection = true;
        overlay.cropSelection = m_selectionPurpose == SelectionPurpose::Crop;
        overlay.selection = m_selection.Rect();
    }

    const HRESULT hr = m_renderer->Present(BackgroundColorLinear(), layoutPtr, overlay);
    if (ImageRenderer::IsDeviceLost(hr))
    {
        m_renderer->DiscardDevice();
        Invalidate(FALSE);
