...
- PNG の tEXt / iTXt は自前パーサで抽出する。**WIC は iTXt 非対応**で、AI ツール（A1111 系）は非 ASCII プロンプトを iTXt に書くため。テキストは UTF-8 として解釈し、失敗時のみ Latin-1（CP1252）にフォールバック（ComfyUI は仕様外ながら tEXt に UTF-8 を書く）。WIC 側の列挙からはテキストチャンクを除外して重複を防ぐ（`/[n]tEXt` 形式や、WIC の unknown ハンドラに包まれた生チャンクも含む）
