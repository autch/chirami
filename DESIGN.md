...
### カラープロファイル（設計決定 2026-09、実装済み — Phase 4 ステップ 28）

画像に埋め込まれた ICC プロファイルを適用して、正しい色で表示する。これまではゲインマップ付き HEIC の Display P3（sRGB カーブの行列型プロファイル）だけを変換し、ほかの画像の ICC は無視していた。そのため、iPhone の JPEG（Display P3）などは色が淡く表示されていた。

**Microsoft Learn で確認したこと:**

- Direct2D の色管理エフェクト（`CLSID_D2D1ColorManagement`）は ICC v4.3 準拠、Windows 8 以降。品質 BEST は機能レベル 10_0 以上と浮動小数点バッファが条件で、浮動小数点の拡張範囲を扱える。変換元は ICC のバイト列から `ID2D1DeviceContext::CreateColorContext(D2D1_COLOR_SPACE_CUSTOM, ...)` で作れ、ヘッダーから sRGB と判定できるものは sRGB として扱われる
- WIC の `IWICColorTransform` は CPU で変換するが、出力は整数形式だけで、sRGB の外側の色は変換先の範囲に切り詰められる

**手元で確認したこと:** WIC は JPEG・HEIC・PNG（iCCP）・WebP のいずれでも ICC を `WICColorContextProfile` として返す（iPhone の JPEG はカラーコンテキストが 2 つで、1 つ目が ICC）。ICC なしの PNG、JPEG XR（FP16）、DNG は返さない。同梱の libjpeg-turbo 3.2 には `tj3GetICCProfile` がある。

**決定事項:**

- **変換は描画時に GPU で行う。** 色管理エフェクトで画像の ICC から scRGB へ変換する。描画先が scRGB（拡張範囲）なので、P3 などの広色域の色も切り詰めずに残り、HDR / 自動色管理のディスプレイでは正しく出る。タイルごとのエフェクトなので追加のメモリは要らない。広色域を失う CPU の WIC 変換は使わない
- **ゲインマップの経路もこれに統一する。** 自前の P3 → sRGB 行列（sRGB カーブのプロファイル限定）を色管理エフェクトに置き換え、どのトーンカーブのプロファイルでも扱えるようにする。`IccProfile` は、sRGB と同等のプロファイルを見分けて変換を省く用途に残す
- **ICC なし・sRGB 同等の ICC の画像は、今の経路のまま。** 表示も性能も変わらない。ICC 付きの画像だけ、タイルを `_SRGB` ではなく UNORM で GPU に送る（色管理エフェクトはガンマの掛かった値を入力に取るため）
- **範囲は 8bit の画像。** 16bit PNG など高精度画像の ICC（今の変換が sRGB 前提でリニア化してしまう）と、PQ / HLG の HDR 画像（既知の制限 1。ICC ではない形で色空間を持つ）は後回し。EXIF の ColorSpace タグで AdobeRGB を示すだけの JPEG も扱わない
- **libjpeg-turbo の経路の ICC は自前で取り出す。** JPEG の APP2（`ICC_PROFILE`）は複数セグメントに分かれうるので、つなぎ直す WTL 非依存のパーサを書き、ユニットテストする。どちらのデコーダでも同じ結果になる
- **保存時は元の ICC を埋め込む。** 画素は元のプロファイルのまま保存されるので、ICC が無いとほかのビューアで色が淡く見える（これまでの保存にもあった問題）。WIC のエンコーダにカラーコンテキストを渡す
- **プロパティウィンドウに「カラープロファイル」の行を足す。** ICC の説明（例: `Display P3`）と、変換したかどうかを出す。「表示形式」「向き」と同じく、読み込み結果（`LoadedImage`）から作る
- **ディスプレイ側のプロファイルは適用しない。** scRGB からディスプレイへの変換は Windows（自動色管理）の役割。自動色管理のない SDR ディスプレイでは sRGB の範囲に切り詰められるが、OS に任せる

**実装:**

- **データ:** `LoadedImage::colorProfile`（`ColorProfile`）が ICC のバイト列・説明・扱い（なし / 変換 / sRGB 相当で変換不要 / 未対応）を持つ。バイト列は RGB のプロファイルのときだけ持つ。CMYK やグレーのプロファイルは、WIC がそれを使わずに RGB へ変換した画素を表していないので、適用も保存時の埋め込みもしない（扱いは「未対応」）
- **読み込み（`ImageLoader`）:** JPEG はバイト列から自前で ICC を取り出し（`ReadJpegIccProfile`。APP2 の分割をつなぎ直す）、libjpeg-turbo と WIC のどちらの経路でも同じものを使う。ほかの形式は WIC のカラーコンテキストのうち最初の ICC を使う。`IsSrgbEquivalent` が sRGB と同等と判定したもの（sRGB カーブは parametric でもテーブルでもよい。Windows 付属の sRGB は 1024 点のテーブル）は「変換不要」。高精度（FP16 経路）の画像は「未対応」
- **描画（`ImageRenderer`）:** 「変換」の画像は ICC から `ID2D1ColorContext` を作り、Direct2D がヘッダーから sRGB と判定したらやはり変換しない。変換するときはタイルを `_SRGB` ではなく UNORM で送り、タイルごとに `ColorManagement`（変換元 = 画像の ICC、変換先 = scRGB、意図は両方とも相対的な色域維持）を掛けて `DrawImage` する。品質は、機能レベル 10_0 以上かつ 32bit 浮動小数点バッファが使えれば BEST（エフェクトの精度も 32bit 浮動小数点に指定）、そうでなければ NORMAL。変換先の scRGB のカラーコンテキストはデバイスごとに 1 つ作っておく
- **ゲインマップとの統一:** タイルのグラフは「`ColorManagement`（ICC があれば）→ ゲインマップの乗算 → 倍率の掛け戻し」。以前の自前の P3 → sRGB 行列は廃止し、`ColorMatrix` は最大値の掛け戻しだけになった
- **保存（`ImageSaver`）:** ICC のバイト列があれば `IWICBitmapFrameEncode::SetColorContexts` で埋め込む。BMP などカラーコンテキストを受け付けないエンコーダでは失敗を記録して、画素だけを保存する（従来どおり）
- **プロパティ:** 「カラープロファイル」の行に「説明（色を変換して表示 / sRGB 相当のため変換なし / 未対応のため変換なし）」を出す。説明が無いプロファイルは「名前なし」

**検証（2026-09、HDR 有効・SDR 白 240 nits のモニタで画面を FP16 のまま取り込み）:**

- P3 の純赤の PNG: scRGB (3.674, -0.126, -0.059) = SDR 白の倍率 3 ×（P3 → sRGB 行列の 1 列目）で理論値どおり。sRGB の外側の色が負の値のまま残る。ICC なしの純赤は (3, 0, 0)
- P3 の純赤の JPEG（ICC を 7 万バイトに水増しして APP2 を 2 分割）: libjpeg-turbo 経由・WIC 経由とも (3.641, -0.125, -0.058)（JPEG の量子化で 255 が 254 程度になる分だけ低い）
- ガンマ 2.2 のプロファイルの灰色 128: 0.659（= 3 × 0.2196、ガンマ 2.2）。sRGB の灰色 128 は 0.6475（= 3 × 0.2158）で、sRGB 以外のトーンカーブも正しく適用される
- Windows 付属の sRGB プロファイルを埋め込んだ画像は「sRGB 相当のため変換なし」で、値は ICC なしと同じ
- 半透明（アルファ 50%）の P3 の赤: 変換後の色 × アルファ + 背景、の計算値とすべての成分で一致。乗算済みアルファは色管理エフェクトが正しく扱う
- 縮小表示の補間: 1 画素ごとの白黒の市松模様を縮小して、ICC なしと P3 付きで同じ値（1.125）。色管理の経路でも、補間はガンマの掛かった値ではなくリニアの値で行われている
- ゲインマップ付き HEIC: HDR の明るさ（最大 7.09）と画素は以前の行列による実装と同じ（差は画面下端に写り込んだタスクバーだけ）
- 保存: P3 の JPEG を PNG と JPEG で保存すると、元の ICC（70,620 バイト）がそのまま埋め込まれ、開き直すと同じ値で表示される。BMP は ICC なしで保存される
...
- ~~既知の制限 2~~（2026-09 解消）: iPhone の HDR 写真（SDR ベース + HDR ゲインマップ構造）は、以前は SDR レンディションでの表示だった。当初の理由は「WIC がゲインマップ（補助画像）を公開しない」だったが、現行の HEIF 拡張では取り出せるため、ゲインマップを合成して HDR で表示するようにした（「HDR ゲインマップ」参照）。ICC プロファイルによる色変換も、8bit の画像すべてに行うようになった（「カラープロファイル」参照）
...
- **ディスプレイへの合わせ方は Skia 方式とする。** ディスプレイの headroom（最大輝度 / SDR 白）に応じた重み W を対数空間で求め、ゲインを `exp(L * W)` として掛ける。Ultra HDR 仕様も同じ考え方で、単純な線形補間は最大ブースト時以外の階調関係を崩すと明記している
  ...
  - ベース画像は Display P3 なので、リニア化して scRGB に移してからゲインを掛ける（ゲインマップ仕様は「ベース画像の色プロファイルが HDR 画像の色空間を定める」としている）。当初は自前の P3 → Rec.709 行列で行い、のちに全画像共通の色管理エフェクトに置き換えた（「カラープロファイル」参照）
...
- **データ:** `LoadedImage` は画素（`PixelBuffer`）に `HdrGainMap`（ゲインマップの画素 + headroom）を足したもの（ベースの色は `ColorProfile`。「カラープロファイル」参照）。ゲインマップは R=G=B に値を入れた Bgra8 で持つ。こうすると回転・反転・クロップ・リサイズと GPU への転送が、通常の画像と同じ処理で済む（メモリは 8bpp の 4 倍になるが、半分の解像度なので 12MP の写真で約 12 MB）
...
- クロップ・リサイズ・黒塗りの後も表示が正しいこと、SDR 表示（HDR オフ。W = 0 で色変換だけが掛かる）で正しく見えることを、ユーザーが実機で確認した
...
- 「カラープロファイル」は、ICC の説明と変換したかどうかを出す（アーキテクチャ方針 >「カラープロファイル」参照）
...
- 当面の対象: パス比較・正規化（`PathCompare`）、拡張子と保存フォーマットの対応（`ImageFormatId`）、WIC の拡張子リストの分割、表示レイアウト計算（`ViewLayout`）、ゲインマップの重みとテーブル（`GainMapMath`）、ICC の判定・説明・JPEG からの取り出し（`IccProfile`）、JPEG の EXIF Orientation の読み取りと回転・反転への対応（`ExifOrientation`）
...
28. カラープロファイルの適用（完了。8bit の画像の ICC を描画時に GPU の色管理エフェクトで scRGB へ変換、ゲインマップの経路も統一、保存時は ICC を埋め込み、プロパティに「カラープロファイル」を表示。詳細はアーキテクチャ方針 >「カラープロファイル」参照）
